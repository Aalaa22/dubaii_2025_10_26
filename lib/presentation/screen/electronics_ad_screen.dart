...
class ElectronicsAdScreen extends StatefulWidget {
  ...
  const ElectronicsAdScreen({Key? key, required this.onLanguageChange})
      : super(key: key);
  @override
            ...
            child: AlertDialog(
              ...
              content: Column(
                ...
                children: [
                  ...
                  ...localizedMissingFields.map((f) => Row(children: [
                        const Icon(
                          Icons.error_outline,
                          color: Colors.red,
                          size: 18,
              ...
              actions: [
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      Navigator.of(context).pop();
                      Future.microtask(() => context.push('/profile'));
                    },
  ...
  Future<void> _pickThumbnailImages() async {
    ...
    if (pickedImages.isNotEmpty) {
      if (pickedImages.length > remainingSlots) {
        ...
        setState(() => _thumbnailImages.addAll(
            pickedImages.sublist(0, remainingSlots).map((e) => File(e.path))));
      } else {
        setState(() =>
            _thumbnailImages.addAll(pickedImages.map((e) => File(e.path))));
      }
    }
  }
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 7.h),
                    Center(
                        child: Text(s.electronicsAndHomeAppliancesAds,
                    ...
                    _buildFormRow([
                      _buildTitledTextFormField(
                          ...
                          hintText: s.area, isRequired: true),
                      _buildTitledTextFormField(
                          s.price, _priceController, borderColor, currentLocale,
                          hintText: '3500', isNumber: true, isRequired: true),
                    ]),
                    const SizedBox(height: 7),
                    _buildFormRow([
                      _buildTitledTextFormField(s.productName,
                          _productNameController, borderColor, currentLocale,
                          hintText: s.enterProductName, isRequired: true),
                      _buildSingleSelectField(
                          context,
                          s.sectionType,
                          selectedSectionType,
                    ...
                    const SizedBox(height: 7),
                    _buildImageButton(
                        '${s.add4Images} (${_thumbnailImages.length}/4)',
          ...
          final result = await showModalBottomSheet<String>(
              ...
              builder: (_) => _SearchableSelectOrAddBottomSheet(
                  ...
                  onAddNew: (newValue) async {
                    ...
                    if (token != null) {
                      final infoProvider =
                          context.read<ElectronicsInfoProvider>();
                      String field;
                      ...
                      final success = await infoProvider
                          .addContactItem(field, newValue, token: token);
                      if (success) {
                        onChanged(newValue);
                      }
                    }
