...
class CarServicesAdScreen extends StatefulWidget {
  ...
  const CarServicesAdScreen({Key? key, required this.onLanguageChange})
      : super(key: key);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(isArabic
              ? 'لقد أضفت الحد الأقصى من الصور ($maxImages صور).'
              : 'You have added the maximum number of images ($maxImages total).'),
  ...
  Future<void> _validateAndProceedToNext() async {
    ...
    if (_descriptionController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              isArabic ? 'الرجاء إدخال الوصف.' : 'Please enter description.'),
          backgroundColor: KPrimaryColor,
        ),
      );
      return;
    ...
    if (selectedLocation.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              isArabic ? 'الرجاء تحديد الموقع.' : 'Please select a location.'),
          backgroundColor: KPrimaryColor,
        ),
      );
      return;
    ...
    final serviceTypeName =
        infoProvider.getServiceNameFromDisplayName(selectedServiceType);
    final emirateName =
        infoProvider.getEmirateNameFromDisplayName(selectedEmirate);
    ...
    print(
        'mainImage: ${adData['mainImage'] != null ? 'File selected' : 'null'}');
    print(
        'thumbnailImages: ${adData['thumbnailImages'] != null ? '${(_thumbnailImages.length)} images' : 'null'}');
                            ...
                            children: [
                              ...
                              _buildFormRow([
                                _buildSingleSelectField(
                                    ...
                                    s!.emirate,
                                    selectedEmirate,
                                    infoProvider.emirateNames, (selection) {
                                  setState(() {
                                    selectedEmirate = selection;
                                    selectedDistrict =
                                        null; // إعادة تعيين المنطقة عند تغيير الإمارة
                                  });
                                }, isRequired: true),
                                _buildSingleSelectField(
                                    context,
                                ...
                                _buildTitledTextFormField(s.area,
                                    _areaController, borderColor, currentLocale,
                              ...
                              _buildFormRow([
                                _buildSingleSelectField(
                                    ...
                                    s.serviceType,
                                    selectedServiceType,
                                    infoProvider.serviceTypeNames, (selection) {
                                  setState(
                              ...
                              const SizedBox(height: 7),
                              _buildTitledTextFormField(s.title,
                              ...
                              Consumer<CarServicesInfoProvider>(
                                builder: (context, provider, child) {
                                  ...
                                  final advertiserNames =
                                      provider.advertiserNames ?? [];
                                  print(
                                      'Building advertiser field with ${advertiserNames.length} names: $advertiserNames');
                                  ...
                                  return TitledSelectOrAddField(
                                    ...
                                    onChanged: (newValue) => setState(() =>
                                        selectedAdvertiserName = newValue),
                              ...
                              _buildFormRow([
                                Consumer<CarServicesInfoProvider>(
                                  builder: (context, provider, child) {
                                    ...
                                    print(
                                        'Building phone field with ${phoneNumbers.length} numbers: $phoneNumbers');
          ...
          validator: isRequired
              ? (value) {
                  if (selectedValue == null)
                    return 'Required'; // التحقق من القيمة المحفوظة في الـ state
                  return null;
                }
              : null,
