...
class CarServicesSaveAdScreen extends StatefulWidget {
  ...
  const CarServicesSaveAdScreen(
      {Key? key, required this.onLanguageChange, this.adId})
      : super(key: key);

  @override
  State<CarServicesSaveAdScreen> createState() =>
      _CarServicesSaveAdScreenState();
}
...
class _CarServicesSaveAdScreenState extends State<CarServicesSaveAdScreen> {
  ...
  Future<void> _fetchAdDetails(int adId) async {
    ...
    debugPrint('[CarServicesSaveAdScreen] _fetchAdDetails: start, adId=' +
        adId.toString());
    try {
      ...
      if (details == null) {
        debugPrint(
            '[CarServicesSaveAdScreen] _fetchAdDetails: API returned null details for adId=' +
                adId.toString());
        setState(() {
          _adData = null;
        });
      ...
      debugPrint('[CarServicesSaveAdScreen] _fetchAdDetails: exception=' +
          e.toString());
      ...
      debugPrint('[CarServicesSaveAdScreen] _fetchAdDetails: end, isLoading=' +
          _isLoading.toString() +
          ', hasData=' +
          (_adData != null).toString());
    }
  }
  ...
  void _populateControllersFromAd() {
    if (_adData == null) {
      debugPrint(
          '[CarServicesSaveAdScreen] _populateControllersFromAd: _adData is null; skip populating controllers');
      return;
    }
    ...
    if (currentTotal >= maxThumbnails) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'لا يمكن إضافة المزيد من الصور. الحد الأقصى $maxThumbnails صور'),
      ...
      if (newImages.length > remainingSlots) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'تم اختيار ${newImages.length} صورة فقط. الحد الأقصى $maxThumbnails صورة إجمالية'),
  ...
  Future<File?> _downloadImageToTempFile(String imageUrl) async {
    try {
      ...
      final filename =
          'car_service_thumb_${DateTime.now().millisecondsSinceEpoch}_${imageUrl.hashCode}.jpg';
  ...
  Future<void> _handleAddNewContact(String field, String value) async {
    ...
    final success =
        await _contactProvider.addContactItem(field, value, token: token);
    if (success) {
  ...
  Future<void> _updateAd() async {
    ...
    try {
      ...
      final keptExistingUrls = _existingThumbnailUrls
          .where((u) => !_removedExistingThumbnailUrls.contains(u))
          .toList();
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Center(
                child: Text(
                  S.of(context)!.carsServicesAds, // استخدام المفتاح الجديد
                  style: TextStyle(
                    fontWeight: FontWeight.w500,
                    fontSize: 24.sp,
                    color: KTextColor,
              ...
              _buildFormRow([
                _buildReadOnlyField(
                    S.of(context)!.emirate, _adData?.emirate ?? ""),
                _buildReadOnlyField(
                    S.of(context)!.district, _adData?.district ?? ""),
                _buildReadOnlyField(S.of(context)!.area, _adData?.area ?? '-'),
              ]),
              const SizedBox(height: 7),

              _buildFormRow([
                _buildReadOnlyField(S.of(context)!.serviceType,
                    _adData?.serviceType ?? serviceTypeValue),
                _buildReadOnlyField(
                    S.of(context)!.serviceName, _adData?.serviceName ?? '-'),
              ...
              _buildReadOnlyField(S.of(context)!.advertiserName,
                  _adData?.advertiserName ?? '-'),
              const SizedBox(height: 7),
              ...
              _buildFormRow([
                TitledSelectOrAddField(
                  ...
                  isNumeric: true,
                  onAddNew: (value) =>
                      _handleAddNewContact('phone_numbers', value),
                ),
                TitledSelectOrAddField(
                  ...
                  isNumeric: true,
                  onAddNew: (value) =>
                      _handleAddNewContact('whatsapp_numbers', value),
                ),
              ]),
              const SizedBox(height: 7),
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  icon:
                      const Icon(Icons.add_a_photo_outlined, color: KTextColor),
                  label: Text(S.of(context)!.addMainImage,
                      style: TextStyle(
                          fontWeight: FontWeight.w600,
                          color: KTextColor,
                          fontSize: 16.sp)),
              ...
              ] else if (_adData?.mainImage != null &&
                  _adData!.mainImage!.isNotEmpty) ...[
                const SizedBox(height: 8),
                ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: Builder(builder: (context) {
                    final mainUrl =
                        ImageUrlHelper.getMainImageUrl(_adData!.mainImage!);
                    ...
                    return Image.asset('assets/images/car_service.png',
                        height: 140, width: double.infinity, fit: BoxFit.cover);
                  }),
                ),
              ],
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  icon: const Icon(Icons.add_photo_alternate_outlined,
                      color: KTextColor),
                  label: Text(S.of(context)!.add3Images,
                      style: TextStyle(
                          fontWeight: FontWeight.w600,
                          color: KTextColor,
                          fontSize: 16.sp)),
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        (() {
                          final loc = (_adData?.location?.trim().isNotEmpty ??
                                  false)
                              ? _adData!.location!.trim()
                              : '${_adData?.emirate ?? ''} ${_adData?.district ?? ''} ${_adData?.area ?? ''}'
                                  .trim();
              ...
              _buildMapSection(
                  context), // دالة الخريطة المأخوذة من الكود السابق
      ...
      children: [
        ...
        if (controller != null)
          TextFormField(
            ...
            style: TextStyle(
                fontWeight: FontWeight.w500,
                color: readOnly ? KDisabledTextColor : KTextColor,
                fontSize: 14.sp),
            textAlign: currentLocale == 'ar' ? TextAlign.right : TextAlign.left,
            decoration: InputDecoration(
              filled: true,
              ...
              border: OutlineInputBorder(
                  ...
                  borderSide: BorderSide(
                      color: readOnly ? Colors.grey.shade400 : borderColor)),
              enabledBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(8),
                  borderSide: BorderSide(
                      color: readOnly ? Colors.grey.shade400 : borderColor)),
              focusedBorder: OutlineInputBorder(
                  borderRadius: BorderRadius.circular(8),
                  borderSide: BorderSide(
                      color: readOnly ? Colors.grey.shade400 : KPrimaryColor,
                      width: readOnly ? 1 : 2)),
              contentPadding: EdgeInsets.all(12),
            ),
          )
                      ...
                      infoWindow: InfoWindow(
                        title: (() {
                          final loc = (_adData?.location?.trim().isNotEmpty ??
                              ...
                              : '${_adData?.emirate ?? ''} ${_adData?.district ?? ''} ${_adData?.area ?? ''}'
                                  .trim();
                          return loc.isNotEmpty ? loc : S.of(context)!.location;
                        })(),
                      ),
                    ),
                  },
