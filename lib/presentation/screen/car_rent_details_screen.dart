...
class CarRentDetailsScreen extends StatefulWidget {
  final String adId;
  const CarRentDetailsScreen({super.key, required this.adId});

  @override
  State<CarRentDetailsScreen> createState() => _CarRentDetailsScreenState();
}

class _CarRentDetailsScreenState extends State<CarRentDetailsScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<CarRentAdProvider>().fetchAdDetails(widget.adId);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<CarRentAdProvider>(
      builder: (context, provider, child) {
        if (provider.isLoadingAdDetails) {
        ...
        if (provider.adDetailsError != null) {
          return Scaffold(
            backgroundColor: Colors.white,
            body: Center(
              child: Column(
                ...
                children: [
                  Text("Error: ${provider.adDetailsError}"),
                  ElevatedButton(
        ...
        final car_rent = provider.currentAd;
        if (car_rent == null) {
          return const Scaffold(
            backgroundColor: Colors.white,
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: PageView.builder(
                            ...
                            itemCount: (car_rent.mainImage != null &&
                                        car_rent.mainImage!.isNotEmpty
                                    ? 1
                                    : 0) +
                                car_rent.thumbnailImages.length,
                            onPageChanged: (index) =>
                              ...
                              if (index == 0 &&
                                ...
                                if (thumbnailIndex >= 0 &&
                                    thumbnailIndex <
                                        car_rent.thumbnailImages.length) {
                                  imageUrl =
                                      ImageUrlHelper.getThumbnailImageUrls(
                                          car_rent
                                              .thumbnailImages)[thumbnailIndex];
                                }
                              }
                        ...
                        Positioned(
                          ...
                          child: buildFavoriteIcon(
                            ...
                            onAddToFavorite: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text('تم إضافة الإعلان للمفضلة')),
                              );
                            },
                          ...
                          child: Row(
                            children: List.generate(
                                (car_rent.mainImage != null &&
                                        ...
                                        ? 1
                                        : 0) +
                                    car_rent.thumbnailImages.length, (index) {
                              return Container(
                        ...
                        children: [
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    SizedBox(width: 6.w),
                                    Text(
                                      '${NumberFormatter.formatPrice(car_rent.price)}',
                                      style: TextStyle(
                                    ...
                                    const Spacer(),
                                    Text(
                                      car_rent.createdAt?.split('T').first ??
                                          'null',
                                      style: TextStyle(
                                        color: Colors.grey,
                                        fontSize: 12.sp,
                                ...
                                SizedBox(height: 6.h),
                                Text(
                                  "${car_rent.make} ${car_rent.model} ${car_rent.trim} ${car_rent.year}",
                                  style: TextStyle(
                                    fontSize: 16.sp,
                                ...
                                Row(
                                  ...
                                  children: [
                                    _buildLabelWithValue(
                                        "Day Rent", car_rent.dayRent),
                                    const SizedBox(width: 16),
                                    _buildLabelWithValue(
                                        "Month Rent", car_rent.monthRent),
                                  ],
                                ),
                                SizedBox(height: 6.h),
                                Text(
                          ...
                          GridView(
                            ...
                            gridDelegate:
                              ...
                              crossAxisCount: 2,
                              mainAxisExtent: Localizations.localeOf(context)
                                          .languageCode ==
                                      'ar'
                            ...
                            children: [
                              _buildDetailBox(S.of(context)!.car_type,
                                  car_rent.carType ?? 'null'),
                              _buildDetailBox(S.of(context)!.trans_type,
                                  car_rent.transType ?? "null"),
                              _buildDetailBox(S.of(context)!.color,
                                  car_rent.color ?? "null"),
                              _buildDetailBox(S.of(context)!.interior_color,
                                  car_rent.interior_color ?? "null"),
                              _buildDetailBox(S.of(context)!.fuel_type,
                                  car_rent.fuelType ?? "null"),
                              _buildDetailBox(S.of(context)!.seats_no,
                                  car_rent.seats_no ?? "null"),
                            ],
                          ),
                          Divider(
                              color: const Color(0xFFB5A9B1), thickness: 1.h),
                          ...
                          Directionality(
                            ...
                            child: Row(
                              children: [
                                Expanded(
                                  child: ReadMoreText(
                                    car_rent.description ?? "null",
                                    trimLines: 5,
                                    colorClickableText:
                                        const Color.fromARGB(255, 9, 37, 108),
                          ...
                          SizedBox(height: 8.h),
                          LocationMap(
                            address: car_rent.location ??
                                '${car_rent.emirate} ${car_rent.area ?? ''}',
                            markerTitle: car_rent.title,
                            height: 188.h,
                          ),
                          SizedBox(height: 10.h),
                          Divider(
                              ...
                              Padding(
                                ...
                                child: Column(
                                  children: [
                                    GestureDetector(
                                      onTap: () {
                                        final phoneNumber = car_rent.whatsapp;
                                        if (phoneNumber != null &&
                                    ...
                                    GestureDetector(
                                      onTap: () {
                                        final phoneNumber =
                                            car_rent.phoneNumber;
                                        if (phoneNumber != null &&
                                            phoneNumber.isNotEmpty) {
                                          final telUrl =
                                              PhoneNumberFormatter.getTelUrl(
                                                  phoneNumber);
                                          _launchUrl(telUrl);
                                        } else {
                                          ScaffoldMessenger.of(context)
...
class CarRentAdItemAdapter implements FavoriteItemInterface {
  ...
  @override
  String get line1 =>
      "Year: ${_carRent.year}  Km: ${NumberFormatter.formatNumber(_carRent.title)}   Specs: ${_carRent.area ?? ''}";
  ...
  @override
  String get title =>
      "${_carRent.make} ${_carRent.model} ${_carRent.trim ?? ''}".trim();
  ...
  AdPriority get priority {
    if (_carRent.planType == null ||
        _carRent.planType!.toLowerCase() == 'free') {
